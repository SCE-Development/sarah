...
module.exports = new Command({
    ...
    execute: async(message, args) => {
        ...
        try {
            ...
            //if there is an error when sending, won't reply with the confirmation msg in the server
            return await message.reply("mamacita check your dm");
        } catch (e) {
            console.log('ik im not your type but can i be the EXCEPTION hehehehe: ', e)
            //if the error object caught has a code of 50007
            //that means that it is the 'cannot send message to the user error'
            if (e.code === 50007) {
                const replyEmbed = new EmbedBuilder()
                    //send an appropriate error message replying to the user in the server
                    .setDescription(`uhhhhhh ${memberName} mamacita, i can't dm you,,,,,, can you check your settings`)
