...
class ReactionHandler {
  ...
  async handleReaction(reaction, user, botpfp, reactionWasRemoved = false) {
    // this makes sure that we are only executing the below 
    // code when a reaction happens
    // on the specific message that we want stuff to happen
    // we define the REACTIONS object in config.json, where 
    // specific messages of our choosing
    // contain maps from emojis to role ids
    // this is basically checking if REACTIONS contains the 
    // message id of the reaction that just happened
    ...
    // "Welcome to SCE discord server react with thumps up if u a real one"
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription('psssst.... hey kid......' + 
          'click on the button for a role......')
        .setFooter({
      ...
      // whenever the button is pressed, an interaction event is 
      // sent to the bot
      ...
      logger.warn('ik im not your type but can i' + 
